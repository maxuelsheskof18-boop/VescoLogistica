...
(function(){
  ...
  function isDelivered(o){
    ...
    return s.includes("entregue") || s.includes("finalizado") || s.includes("concluid");
  }
  ...
  async function loadData(force=false){
    ...
    try{
      const main=await jsonp(API_MAIN,{action:"loadVesco",dataISO:state.date,mes:state.month},22000);
      ...
      console.warn("V8.2: API ERP não respondeu; usando dados locais.",e.message);
      orders=localOrders().map(o=>normalizeOrder(o,"erp")).filter(o=>!o.is_flex);
    }

    try{
      const fp=await jsonp(API_FLEX,{action:"enviosFlex",dataISO:state.date,mes:state.month,allFlex:"1"},120000);
      ...
      const rejected=apiFlex.filter(o=>!( !o.is_delivered && o.is_flex && isFlexProjeto(o) ));
      state.lastFlexAcceptedCount=accepted.length;
      state.lastFlexRejectedSamples=rejected.slice(0,5).map(o=>({
        pedido:number(o),
        ecom:ecom(o),
        conta:txt(pick(o,["conta","loja","store_name"])),
        marcador:flexMarker(o),
        source:txt(pick(o,["__v8source","__source"])),
        validado:flexValidated(o),
        status:status(o)
      }));
      if(apiFlex.length){
        flex=accepted;
        saveStoredFlex(flex, state.month);
        // Também salva por mês real do pedido, já limpo.
        const byMonth={};
        flex.forEach(o=>{ const m=flexMonthOf(o); if(m){ (byMonth[m]||(byMonth[m]=[])).push(o); }});
        Object.keys(byMonth).forEach(m=>saveStoredFlex(byMonth[m],m));
      }
    }catch(e){
      console.warn("V8.7: API Flex não respondeu; por segurança não uso armazenamento antigo.",e.message);
      flex=[];
    }
...
function layout(){
  ...
  function routeMotoristaLink(r){
    ...
    url.searchParams.set("token", txt(r.token || ""));
  ...
  function renderProntoEnvio(){
    ...
    content.innerHTML=`
      ${kpis([
        ...
        {label:"Mapa interno",value:String(list.filter(coords).length),small:"com lat/lon"}
      ])}
  ...
  async function salvarRotaSelecionada(){
    ...
    try{
      ...
      await loadData(true);
      renderProntoEnvio();
      const rotaObj=resp.rota || resp.data || resp;
      if(rotaObj){
        const app=routeMotoristaLink(rotaObj);
        try{ await navigator.clipboard.writeText(app); }catch(e){}
        alert("Rota salva com sucesso. Link do motorista copiado.");
      }else{
  ...
  async function updateStatus(id,statusNovo){
    try{
      showLoading(true);
      const resp=await jsonp(API_MAIN,{action:"updateStatus",id,status:statusNovo,operador:"Painel"},16000);
      showLoading(false);
...
function render(){
  ...
  window.VescoV8={__v82:true,__v821:true,__v84:true,__v86:true,__v861:true,__v87:true,__v871:true,__v872:true,__v873:true,__v874:true,__v875:true,__v876:true,__v90:true,__v91:true,__v92:true,state,init,go,
    ...
    sleep,
    routeReadyList,routeFlexExtras,addFlexToRouteByCode,removeFlexFromRoute,
