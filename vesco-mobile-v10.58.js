// VESCO V10.58 — mobile com scroller único e paginação realmente responsiva.
// Nenhuma regra desta camada modifica o desktop acima de 899px.
(function(){
  "use strict";
  if(window.VescoMobileV1058) return;
  ...
  function scrollDiagnostics(){
    const scroller=mobileScroller();
    return {
      version:"V10.58-MOBILE-SCROLLER-UNICO",
      mobile:isMobile(),
  ...
  function applyViewport(){
    ...
    document.body.classList.toggle("v1058-keyboard-open",!!keyboard);
    document.body.classList.toggle("v1058-mobile-scroller",isMobile());
  }

  function setupTopbar(){
    if(!isMobile()) return;
    const actions=q("#v8Topbar .v8-top-actions");
    if(!actions||actions.dataset.v1058Ready==="1") return;
    actions.dataset.v1058Ready="1";
    ...
    quick.className="v1058-mobile-quick";
    const panel=document.createElement("div");
    panel.className="v1058-filter-panel";
    panel.id="v1058FilterPanel";
    const toggle=document.createElement("button");
    toggle.type="button";
    toggle.id="v1058FiltersToggle";
    toggle.className="v8-btn secondary v1058-filter-toggle";
    toggle.setAttribute("aria-controls","v1058FilterPanel");
    ...
    const open=safeGet(sessionStorage,"vesco:v1058:filtersOpen")==="1";
    document.body.classList.toggle("v1058-filters-open",open);
    toggle.setAttribute("aria-expanded",open?"true":"false");
    toggle.addEventListener("click",()=>{
      const next=!document.body.classList.contains("v1058-filters-open");
      document.body.classList.toggle("v1058-filters-open",next);
      toggle.setAttribute("aria-expanded",next?"true":"false");
      safeSet(sessionStorage,"vesco:v1058:filtersOpen",next?"1":"0");
    });
  }
  ...
  function restoreDesktopTopbar(){
    ...
    delete actions.dataset.v1058Ready;
  }

  function setupSidebar(){
    const sidebar=q("#v8Sidebar");
    if(!sidebar||sidebar.dataset.v1058Ready==="1") return;
    sidebar.dataset.v1058Ready="1";
    let close=q(".v1058-sidebar-close",sidebar);
    if(!close){
      close=document.createElement("button");
      close.type="button";
      close.className="v1058-sidebar-close";
  ...
  function labelTable(table){
    ...
    table.classList.add("v1058-card-table");
  ...
  function setupObserver(){
    const content=q("#v8Content");
    if(!content||content.dataset.v1058Observed==="1") return;
    content.dataset.v1058Observed="1";
  ...
  function bindNavigationAftercare(){
    if(document.documentElement.dataset.v1058NavAftercare==="1") return;
    document.documentElement.dataset.v1058NavAftercare="1";
    ...
    document.addEventListener("click",ev=>{
      ...
      if(!nav) return;
      if(nav.dataset.tab!=="menu") document.body.classList.remove("v8-mobile-menu-open");
  ...
  function applyMode(){
    ...
    applyViewport();
    if(mobile){
      document.body.classList.add("v1058-mobile-scroller");
      setupTopbar();
      setupSidebar();
      queueEnhance();
      clearInteractionLocks();
    }else{
      document.body.classList.remove("v1058-mobile-scroller","v1058-filters-open","v1058-keyboard-open","v8-mobile-menu-open");
  ...
  window.VescoMobileV1058={
    version:"V10.58-MOBILE-SCROLLER-UNICO",
    isMobile,mobileScroller,resetScrollTop,scrollDiagnostics,applyMode,enhanceContent
  };
  if(document.readyState==="loading") document.addEventListener("DOMContentLoaded",boot,{once:true});
  else boot();
  console.log("VESCO Mobile V10.58 ativo — scroller único, paginação real e desktop intacto.");
})();
