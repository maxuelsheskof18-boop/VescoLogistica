// vesco-firebase-realtime.js — V10.39 WORKER LIGHT
  ...
  function start(){
    ...
    if(window.VescoV8 && typeof window.VescoV8.startRealtimeSync==='function'){
      ...
      window.VescoFirebaseRealtime={
        version:'V10.39-worker-light',
        start:()=>window.VescoV8.startRealtimeSync(),
        stop:()=>window.VescoV8.stopRealtimeSync(),
        status:()=>window.VescoV8.realtimeStatus(),
        result
      };
      console.log('VESCO Firebase Realtime V10.39 Light ativo — sem sincronização duplicada de rotas.');
      return true;
    }
    if(attempts>80){
      clearInterval(timer);
      timer=null;
      console.warn('VESCO Firebase Realtime V10.39: núcleo VescoV8 não encontrado.');
    }
    return false;
  }
