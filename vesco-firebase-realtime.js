// vesco-firebase-realtime.js — VESCO CONTROL V10.42 FIREBASE LIMPO
...
(function(){
  if(window.__VESCO_FIREBASE_REALTIME_V1042) return;
  window.__VESCO_FIREBASE_REALTIME_V1042 = true;
  ...
  async function ensureFirebase(){
    ...
    initialized = true;
    console.log("VESCO Firebase V10.42 conectado:", cfg.databaseURL);
    return db;
  }
  ...
  function applyOrderPatchToState(id, patch){
    ...
    // V10.42: não cria mais pedidos fantasmas no state.orders.
  ...
  function applyRoutesToState(routesObj){
    ...
    routes.forEach(r=>{
      ...
      Object.keys(entregas).forEach(k=>{
        ...
        // V10.42: mesmo se o pedido não estiver carregado no ERP, aparece em Entregues.
        const v8=getV8();
        if(v8 && v8.state){
...
async function startListeners(){
  ...
  function patchVescoV8Methods(){
    ...
    // V10.42: não sobrescreve updateStatus do módulo principal.
    ...
    console.log("VESCO Firebase Realtime V10.42: métodos do painel sincronizados.");
  }

  async function boot(){
    const wait=()=>new Promise(r=>setTimeout(r,200));
    for(let i=0;i<80;i++){
      if(window.VescoV8 && window.VescoV8.state) break;
      await wait();
    }
    if(!(window.VescoV8 && window.VescoV8.state)){
      console.warn("VESCO Firebase Realtime V10.42: aguardando núcleo VescoV8; listeners serão tentados depois.");
      setTimeout(()=>boot().catch(e=>console.warn("VESCO Firebase V10.42 retry:", e.message||e)), 3000);
      return false;
    }
  ...
  // V10.42: realtime pesado desligado por padrão.
  ...
  if(window.VESCO_FIREBASE_REALTIME_ENABLED === true){
    boot().catch(e=>console.error("VESCO Firebase V10.42 erro:", e));
  }else{
    console.log("VESCO Firebase Realtime V10.42 em modo leve: listeners automáticos desligados.");
  }
})();
