...
(function(){
  "use strict";
  if(window.VescoMobileV1054) return;
  ...
  function clearInteractionLocks(){
    document.body.classList.remove("v1051-nav-busy","v1054-touch-active");
  ...
  function applyViewportVars(){
    ...
    if(keyboard!==lastViewport.keyboard){
      document.body.classList.toggle("v1054-keyboard-open",keyboard);
      document.body.classList.toggle("v1052-keyboard-open",keyboard);
      lastViewport.keyboard=keyboard;
    }
    document.body.classList.toggle("v1054-is-mobile",isMobile());
  }
  ...
  function setupTopbar(){
    ...
    if(!actions || actions.dataset.v1054Ready==="1") return;
    actions.dataset.v1054Ready="1";
    ...
    quick.className="v1054-mobile-quick";
    const panel=document.createElement("div");
    panel.className="v1054-filter-panel";
    panel.id="v1054FilterPanel";
    const toggle=document.createElement("button");
    toggle.type="button";
    toggle.id="v1054FiltersToggle";
    toggle.className="v8-btn secondary v1054-filter-toggle";
    toggle.setAttribute("aria-controls","v1054FilterPanel");
    ...
    const open=safeGet(sessionStorage,"vesco:v1054:filtersOpen")==="1";
    document.body.classList.toggle("v1054-filters-open",open);
    toggle.setAttribute("aria-expanded",open?"true":"false");
    toggle.addEventListener("click",ev=>{
      ev.preventDefault();
      const next=!document.body.classList.contains("v1054-filters-open");
      document.body.classList.toggle("v1054-filters-open",next);
      toggle.setAttribute("aria-expanded",next?"true":"false");
      safeSet(sessionStorage,"vesco:v1054:filtersOpen",next?"1":"0");
  ...
  function setupSidebar(){
    ...
    if(!sidebar || sidebar.dataset.v1054Ready==="1") return;
    sidebar.dataset.v1054Ready="1";
    sidebar.setAttribute("aria-label","Menu principal");
    let close=q(".v1054-sidebar-close",sidebar);
    if(!close){
      close=document.createElement("button");
      close.type="button";
      close.className="v1054-sidebar-close";
  ...
  function labelTable(table){
    ...
    table.classList.add("v1054-card-table");
  ...
  function setupObserver(){
    const content=q("#v8Content");
    if(!content || content.dataset.v1054Observed==="1") return;
    content.dataset.v1054Observed="1";
  ...
  function setupNavigationGuard(){
    if(document.documentElement.dataset.v1054NavGuard==="1") return;
    document.documentElement.dataset.v1054NavGuard="1";
    ...
    document.addEventListener("click",ev=>{
      ...
      requestAnimationFrame(()=>{
        try{
          ...
          console.error("V10.54: navegação mobile recuperada",error);
    ...
    document.addEventListener("touchstart",()=>{
      document.body.classList.add("v1054-touch-active");
      setTimeout(()=>document.body.classList.remove("v1054-touch-active"),240);
    },{passive:true});
  ...
  function setupShell(){
    ...
    shellReady=true;
    document.documentElement.classList.add("vesco-v1054-mobile-capable");
    document.body.classList.add("v1054-mobile-ready");
    setupTopbar();
    setupSidebar();
    setupObserver();
  ...
  document.addEventListener("keydown",ev=>{
    ...
    closeMenu();
    document.body.classList.remove("v1054-filters-open");
    q("#v1054FiltersToggle")?.setAttribute("aria-expanded","false");
    clearInteractionLocks();
  });

  window.VescoMobileV1054={
    version:"V10.54-MOBILE-SCROLL-SEGURO",
    setupShell,enhanceContent,queueViewport,isMobile,clearInteractionLocks,releaseScrollLock,scrollDiagnostics
  };
  if(document.readyState==="loading") document.addEventListener("DOMContentLoaded",boot,{once:true});
  else boot();
  console.log("VESCO Mobile V10.54 ativo — scroll nativo restaurado, toque destravado e navegação resiliente.");
})();
